...
interface PlantData {
  ...
  name: string;
  image: string;
  needsWaterParagraph: () => React.ReactNode;
  needsLightParagraph: () => React.ReactNode;
  isIndoorParagraph: () => React.ReactNode;
  description: string;
  comments: Comment[];
}
...
function Show({ data }: ShowProps): JSX.Element {
  ...
  if (data.plant.comments.length) {
    comments = data.plant.comments.map((c, index) => (
  ...
  return (
    <Def>
      <main>
        ...
        <h1>Comments</h1>
        {comments}
        <form method="POST" action={`/plants/${data.plant.id}/comment`} onSubmit={handleSubmit}>
