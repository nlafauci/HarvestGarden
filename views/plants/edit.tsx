...
function EditForm({ data }: EditFormProps): JSX.Element {
  const handleLightChange = (event: ChangeEvent<HTMLSelectElement>): void => {
      ...
      <main>
        ...
        <form method="POST" action={`/plants/${data.plant.id}?_method=PUT`}>
          ...
          <div>
            ...
            <select id="needsWater" name="needsWater" value={data.plant.needsWater} required>
          ...
          <div>
            ...
            <input
              id="description"
              name="description"
              defaultValue={data.plant.description}
              required
            />
          </div>
